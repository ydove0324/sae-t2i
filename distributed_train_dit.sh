...
# ==================================================
# 多机并行训练脚本 - DiT 训练（支持不同SSH端口）
# ==================================================
# 使用方法：
#   1. 配置 WORKER_NODES 和对应的 SSH_PORTS
#   2. 配置节点信息（GPU数量、MASTER地址等）
#   3. 运行: bash distributed_train_dit.sh

# =================== 节点配置 ===================
# Worker节点列表（按顺序，rank从1开始）
WORKER_NODES=(
    "root@139.224.222.61"  # 示例：机器1
    # "root@139.224.222.62"  # 示例：机器2
    # "root@139.224.222.63"  # 示例：机器3
)
...
MASTER_PORT="27597"  # 确保与VAE训练使用不同的端口
JOB_ID="200"

# =================== 脚本配置 ===================
TRAIN_SCRIPT="script/run_kl500_dit.sh"
LOG_FILE="log/dit_kl500_multi_node.log"
TMUX_SESSION="dit_kl500_multi_node"
...
# =================== Master节点启动 ===================
echo "=========================================="
echo "Starting Multi-Node Distributed DiT Training"
echo "=========================================="
...
for i in "${!WORKER_NODES[@]}"; do
    ...
    ssh -p $SSH_PORT $NODE "bash -c \"
        ...
        tmux send-keys -t '$TMUX_SESSION' 'export NODE_RANK=$WORKER_RANK && export MASTER_ADDR=$MASTER_ADDR && export MASTER_PORT=$MASTER_PORT && export NUM_NODES=$NUM_NODES && export NUM_GPUS=$WORKER_GPUS && export JOB_ID=$JOB_ID && source $CONDA_PATH && conda activate $CONDA_ENV && cd $WORKSPACE_PATH && bash $TRAIN_SCRIPT' C-m
    \"" &
...
echo ""
echo "=========================================="
echo "Distributed DiT training completed!"
echo "=========================================="
