...
# Two-stage: 1) Extract and save features, 2) Train linear classifier
...
class FeatureDataset(Dataset):
    ...
    def __init__(self, features_path: str, labels_path: str = None):
        self.features = np.load(features_path)
        if labels_path is not None:
            self.labels = np.load(labels_path)
        else:
            # Try to load labels from same directory
            labels_path = features_path.replace("_features.npy", "_labels.npy")
    ...
    def __getitem__(self, idx):
        return torch.from_numpy(self.features[idx]).float(), int(self.labels[idx])
...
@torch.no_grad()
def extract_features_batch(vae, x: torch.Tensor) -> torch.Tensor:
    """Extract avg-pooled features from VAE encoder."""
    feat = vae.encode_features(x, use_lora=True)  # [B, C, H, W]
    features = feat.mean(dim=[2, 3])  # [B, C] - avg pooling
    return features
...
def extract_and_save_features(
    ...
    for x, y in iterator:
        x = x.to(device)
        features = extract_features_batch(vae, x)
    ...
    if world_size > 1:
        # Save each rank's features
        rank_features_path = output_path.replace(".npy", f"_rank{rank}.npy")
        rank_labels_path = output_path.replace("_features.npy", f"_labels_rank{rank}.npy")
        np.save(rank_features_path, all_features)
        np.save(rank_labels_path, all_labels)
        ...
        if rank == 0:
            ...
            for r in range(world_size):
                rf = output_path.replace(".npy", f"_rank{r}.npy")
                rl = output_path.replace("_features.npy", f"_labels_rank{r}.npy")
                merged_features.append(np.load(rf))
                merged_labels.append(np.load(rl))
                os.remove(rf)
                os.remove(rl)
            
            all_features = np.concatenate(merged_features, axis=0)
            all_labels = np.concatenate(merged_labels, axis=0)
    
    # Save final features (rank 0 only)
    if rank == 0:
        np.save(output_path, all_features)
        labels_path = output_path.replace("_features.npy", "_labels.npy")
        np.save(labels_path, all_labels)
        print(f"Saved features: {output_path} ({all_features.shape})")
        print(f"Saved labels: {labels_path} ({all_labels.shape})")
    
    dist.barrier()
    return all_features.shape[1]  # feature dim


def get_feature_dim(vae) -> int:
    """Get the feature dimension (avg pooling)."""
    return vae.original_latent_channels
...
def train_one_epoch(
    ...
    """Train linear classifier for one epoch using pre-extracted features."""
    classifier.train()
...
def evaluate(
    ...
    """Evaluate linear classifier on validation set using pre-extracted features."""
    classifier.eval()
...
def main():
    ...
    parser.add_argument("--mode", type=str, default="all", choices=["extract", "train", "all"],
                        help="Mode: 'extract' features only, 'train' from saved features, or 'all' (both)")
    ...
    parser.add_argument("--train-features", type=str, default=None, 
                        help="Path to pre-extracted train features. Auto-generated if not specified.")
    parser.add_argument("--val-features", type=str, default=None,
                        help="Path to pre-extracted val features. Auto-generated if not specified.")
    ...
    # Auto-generate feature paths
    if args.train_features is None:
        args.train_features = os.path.join(args.output_dir, "train_features.npy")
    if args.val_features is None:
        args.val_features = os.path.join(args.output_dir, "val_features.npy")
    ...
    if args.mode in ["extract", "all"]:
        ...
        train_labels_path = args.train_features.replace("_features.npy", "_labels.npy")
        val_labels_path = args.val_features.replace("_features.npy", "_labels.npy")
        
        train_features_exist = os.path.exists(args.train_features) and os.path.exists(train_labels_path)
        val_features_exist = os.path.exists(args.val_features) and os.path.exists(val_labels_path)
        
        if train_features_exist and val_features_exist:
            if rank == 0:
                print("\n" + "=" * 50)
                print(" Stage 1: Feature Extraction (SKIPPED)")
                print("=" * 50)
                print(f"  Train features already exist: {args.train_features}")
                print(f"  Val features already exist: {args.val_features}")
                print("  Use --mode extract to force re-extraction")
        else:
            if args.vae_ckpt is None:
                raise ValueError("--vae-ckpt is required for feature extraction")
            ...
            # Create datasets for extraction (no augmentation for train, center crop for both)
            ...
            # Helper class for flat image folder (no subdirectories)
            ...
            # Extract train features (if not exist)
            if not train_features_exist:
                if rank == 0:
                    print("\nExtracting train features...")
                
                try:
                    train_dataset = ImageFolder(args.train_path, transform=extract_transform)
                except:
                    train_dataset = ImageDataset(args.train_path, transform=extract_transform)
                ...
                feature_dim = extract_and_save_features(
                    vae, train_loader, device, args.train_features, rank, world_size
                )
            else:
                if rank == 0:
                    print(f"\nTrain features already exist, skipping: {args.train_features}")
            
            # Extract val features (if not exist)
            if not val_features_exist:
                if rank == 0:
                    print("\nExtracting val features...")
                
                try:
                    val_dataset = ImageFolder(args.val_path, transform=extract_transform)
                except:
                    val_dataset = ImageDataset(args.val_path, transform=extract_transform)
                ...
                extract_and_save_features(
                    vae, val_loader, device, args.val_features, rank, world_size
                )
            else:
                if rank == 0:
                    print(f"\nVal features already exist, skipping: {args.val_features}")
            ...
            if rank == 0:
                ...
                print(f"  Val features: {args.val_features}")
    ...
    if args.mode in ["train", "all"]:
        ...
        if not os.path.exists(args.train_features):
            raise FileNotFoundError(f"Train features not found: {args.train_features}")
        if not os.path.exists(args.val_features):
            raise FileNotFoundError(f"Val features not found: {args.val_features}")
        
        # Load feature datasets
        if rank == 0:
            print("\nLoading pre-extracted features...")
        
        train_feature_dataset = FeatureDataset(args.train_features)
        val_feature_dataset = FeatureDataset(args.val_features)
        
        feature_dim = train_feature_dataset.features.shape[1]
        
        if rank == 0:
            print(f"Feature dimension: {feature_dim}")
            print(f"Train samples: {len(train_feature_dataset)}")
            print(f"Val samples: {len(val_feature_dataset)}")
        
        # Create data loaders
        train_sampler = DistributedSampler(train_feature_dataset, shuffle=True)
        val_sampler = DistributedSampler(val_feature_dataset, shuffle=False)
        ...
        for epoch in range(start_epoch, args.epochs):
            ...
            train_loss, train_acc = train_one_epoch(
                classifier, train_loader, optimizer, device, epoch, rank
            )
            
            # Evaluate
            val_top1, val_top5 = evaluate(
                classifier, val_loader, device, rank
            )
