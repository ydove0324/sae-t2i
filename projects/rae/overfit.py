...
def main(args):
    ...
    if rank == 0:
        ...
        experiment_name = (
            f"{args.ae}_x{args.sdown}_KL{args.KL}_fit_{experiment_index:03d}-{model_string_name}-"
            f"{path_type}-{prediction}-{loss_weight_str}{precision_suffix}-acc{grad_accum_steps}_channel{args.channel}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ae", type=str, default="sae")
    parser.add_argument("--sdown", type=int, default=16)
    parser.add_argument("--KL", type=int, default=0)
    parser.add_argument("--channel", type=int, default=1280)
