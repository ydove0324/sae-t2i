...
from models.dino_v3.configuration_dinov3_vit import DINOv3ViTConfig
...
class DINOv3ViTEmbeddings(nn.Module):
    ...
    def __init__(self, config: DINOv3ViTConfig):
        ...
        # patch size = 16, wo = [(wi - kernal) / s] + 1 = (768-16)/16 + 1 = 48，得到 48 x 48 的kernal 

    def forward(self, pixel_values: torch.Tensor, bool_masked_pos: Optional[torch.Tensor] = None) -> torch.Tensor:
        batch_size = pixel_values.shape[0]
        # print(f"here is the pixal value shape {pixel_values.shape}")
        ...
        # print(f"here is the patch_embeddings shape {patch_embeddings.shape}")
        patch_embeddings = patch_embeddings.flatten(2).transpose(1, 2)
         #print(f"here is the 2nd patch_embeddings shape {patch_embeddings.shape}")

        # print(f"bool_masked_pos is {bool_masked_pos}")
        ...
        # print(f"here is the cls_token shape {cls_token.shape}")
        register_tokens = self.register_tokens.expand(batch_size, -1, -1)
        # print(f"here is the register_tokens shape {register_tokens.shape}")
        embeddings = torch.cat([cls_token, register_tokens, patch_embeddings], dim=1)
        # print(f"here is the embeddings shape {embeddings.shape}")
        # print(f"here is the embedding content {embeddings}")
